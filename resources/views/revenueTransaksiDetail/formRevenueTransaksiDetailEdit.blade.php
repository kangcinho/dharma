...
@section('content')
	<div class="ui breadcrumb">
	  ...
	  <a class="active section" href="{{ route('revenue list perbulan', $idx) }}">Kembali</a>
	</div>
	<h2 class="ui header dividing">{{ 'Form Revenue Transaksi ' . $tahunRevenue->revenue->tahun->tahun . '-' . $bulanRevenue->bulan->bulan }}</h2>
	<form class="ui form" action="{{ route('edit save revenue transaksi detail',['id' =>$revenueTransaksiDetail->id, 'idx' =>  $idx]) }}" method="POST" id="formRevenue">
		@csrf
		<div class="required field">
			<label for="idTahun">Kategori {{ $revenueTransaksiDetail->idKategori }}</label>
			<div class="ui selection dropdown" >
			  <input type="hidden" name="idKategori" id="idKategori" value="{{ $revenueTransaksiDetail->idKategori }}">
		...
		<div class="required field">
			...
			<input type="text" name="totalRevenue" id="totalRevenue" value="{{ $revenueTransaksiDetail->totalRevenue }}">
		</div>
