...
class PasienRepeaterDetailController extends Controller
{
    public function dashboard($idSectionSanata){
        ...
        $tahunX_2 = $this->dataPasienRepeater($idSectionSanata, $tahunAktif-2);
        $tahunX_1 = $this->dataPasienRepeater($idSectionSanata, $tahunAktif-1);
        $tahunX = $this->dataPasienRepeater($idSectionSanata, $tahunAktif);

        $this->chartJumlahPasien($tahunX, $tahunX_1, $tahunX_2, $idSectionSanata, $tahunAktif);
        return view('menu.pasienRepeater.pasienRepeaterDetail.indexPasienRepeaterDetail', compact('tahunX','lastUpdate'));
    }

    public function dataPasienRepeater($idSectionSanata, $tahunAktif){
        $dataPasienRepeater = RevenueTransaksiDetail::selectRaw('mKategori.namaKategori AS namaKategori, mTahun.tahun AS tahun, mbulan.bulan AS bulan, mbulan.bulanSingkatan AS bulanSingkatan, trRevenueTransaksiDetail.totalRevenue AS totalRevenue, trRevenueTransaksiDetail.jumlahPasienTotal AS jumlahPasien ')
    ...
    public function dashboardDaily($idSectionSanata){
        ...
        if($idSectionSanata == 1){
            $idSectionSanata = 'RI';
            $kategori->namaKategori = "Pasien Repeater RI";
        }else{
            $idSectionSanata = 'RJ';
            $kategori->namaKategori = "Pasien Repeater RJ";
        }

        $dataPasienBaru = $this->dataPasienRepeaterDaily($tahunSekarang, $bulanSekarang, $idSectionSanata);
        $dataPasienBaru_1 = $this->dataPasienRepeaterDaily($tahunSekarang-1, $bulanSekarang, $idSectionSanata);
        $dataPasienBaru_2 = $this->dataPasienRepeaterDaily($tahunSekarang-2, $bulanSekarang, $idSectionSanata);

        $this->chartJumlahPasienDaily($dataPasienBaru, $dataPasienBaru_1, $dataPasienBaru_2, $tglSekarang);

        return view('menu.pasienRepeater.pasienRepeaterDetail.indexPasienRepeaterDetailDaily', compact('tglSekarang','kategori'));
    }

    public function dataPasienRepeaterDaily($tahunSekarang, $bulanSekarang, $idSectionSanata){
        $dataPasienRepeaterDaily = \DB::connection('sqlsrv')->table('SIMtrKasir')
            ...
            ->get();
        return $dataPasienRepeaterDaily;
    }
